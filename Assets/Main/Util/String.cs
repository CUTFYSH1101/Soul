...
namespace Main.Util
{
    public static class String
    {
        ...
        public static string[] GetFieldInfosToNameString(this object source) =>
            source.GetFieldInfos().Select(p => p.Name + "\t" + p.GetValue(source)).Where(arg => arg != "").ToArray();

        public static string[] GetPropertiesInfosToNameString(this object source) =>
            source.GetPropertiesInfos().Select(p => p.Name + "\t" + p.GetValue(source)).Where(arg => arg != "").ToArray();
    }
}